...
export async function GET(req: NextRequest) {
  ...
  const result: any = { version: 'v9-anticaptcha', steps: [] };
  ...
  try {
    ...
    // Step 2: Solve hCaptcha via anti-captcha.com
    const anticaptchaKey = process.env.ANTICAPTCHA_KEY;
    result.anticaptchaKey = anticaptchaKey ? `SET (len=${anticaptchaKey.length})` : 'NOT SET';

    if (!anticaptchaKey || !anticaptchaKey.trim()) {
      result.steps.push(`${ms()} ANTICAPTCHA_KEY not set in Render env vars`);
      throw new Error('ANTICAPTCHA_KEY not configured');
    }

    result.steps.push(`${ms()} creating hCaptcha task on anti-captcha.com...`);
    const createResp = await axios.post('https://api.anti-captcha.com/createTask', {
    ...
    result.steps.push(`${ms()} createTask: errorId=${createResp.data?.errorId} taskId=${createResp.data?.taskId}`);

    const taskId = createResp.data?.taskId;
    if (createResp.data?.errorId !== 0 || !taskId) {
      throw new Error('createTask failed: ' + createResp.data?.errorDescription);
    }

    // Step 3: Poll for hCaptcha solution
    result.steps.push(`${ms()} polling for solution...`);
    let token: string | null = null;
    for (let i = 0; i < 20; i++) {
      await new Promise(r => setTimeout(r, 3000));
      const pollResp = await axios.post('https://api.anti-captcha.com/getTaskResult',
        { clientKey: anticaptchaKey, taskId },
        { timeout: 8000 }
      );
    ...
    if (!token) throw new Error('hCaptcha solving timed out or failed');

    // Step 4: Call generate/v2 with real hCaptcha token
    ...
    result.clipIds = genResp.data?.clips?.map((c: any) => c.id);
